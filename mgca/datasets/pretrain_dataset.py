...
class MultimodalPretrainingDataset(data.Dataset):
    def __init__(self, is_client = -1, split="train", transform=None, data_pct=1.0,
                 imsize=256, max_words=112, sent_num=3):
        # is_client(int): -1->server, 0: client_0, 1: client_1,......
        super().__init__()
        ...
        if is_client == -1:
            ...
            self.df = pd.read_csv(str(MIMIC_CXR_DATA_DIR)+ "/client_" + str(is_client) + "_master.csv")
        
        self.df = self.df[self.df["ViewPosition_x"].isin(["PA", "AP"])] # ViewPosition_x and ViewPosition_x are the same
        ...
        print("length of self.filenames:",len(self.filenames))
        self.df = self.df[self.df[MIMIC_CXR_SPLIT_COL] == split]
        if data_pct != 1.0 and split == "train":
            print(f"sample data with ratio {data_pct}")
