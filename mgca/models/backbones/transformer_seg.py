...
class Encoder2D_MAE(nn.Module):
    ...
    def forward(self, x):
        ...
        latent_mae, mask_mae, ids_restore_mae = self.bert_model.forward_encoder(x, 0)

        encode_x = latent_mae[:, 1:]
        if not self.is_segmentation:
            return encode_x
...
class SETRModel_MAE(nn.Module):
    def __init__(self, patch_size=(32, 32),
        ...
        config = TransConfig(patch_size=patch_size,
                             ...
                             num_hidden_layers=num_hidden_layers,
                             num_attention_heads=num_attention_heads)
        self.encoder_2d = Encoder2D_MAE(config)
        self.decoder_2d = Decoder2D(in_channels=config.hidden_size,
                                    out_channels=config.out_channels, features=decode_features)
